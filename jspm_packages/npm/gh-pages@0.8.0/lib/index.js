...
(function(process) {
  ...
  function getRemoteUrl(dir, remote) {
    ...
    return git(['config', '--get', 'remote.' + remote + '.url'], dir).progress(function(chunk) {
      ...
      return Q.reject(new Error('Failed to get remote.origin.url (task must either be run in a ' + 'git repository with a configured origin remote or must be ' + 'configured with the "repo" option).'));
    });
  }
