...
(function(Buffer) {
    ...
    return Q.fcall(function() {
      ...
      if (write || append) {
        ...
        if ("begin" in options && "end" in options) {
          return new BufferStream([Reader.join(fileNode._chunks).slice(options.begin, options.end)], charset);
        } else {
          return new BufferStream(fileNode._chunks, charset);
        }
      }
  ...
  MockFs.prototype.rename = function(source, target) {
    ...
    return Q.fcall(function() {
      ...
      var targetNode = targetDirectoryNode._entries[targetName];
      if (targetNode) {
  ...
  var stats = ["isDirectory", "isFile", "isBlockDevice", "isCharacterDevice", "isSymbolicLink", "isFIFO", "isSocket"];
  ...
  DirectoryNode.prototype._canonicalParts = function(parts, via) {
    ...
    return this._entries[part]._canonicalParts(parts, this._fs.join(via, part));
  };
  ...
  LinkNode.prototype._canonicalParts = function(parts, via) {
    return this._fs.relativeFromDirectory(this._fs.ROOT, this._fs._root._canonical(this._fs.absolute(this._fs.join(via, "..", this._link))));
  };
  var FS = require('./fs');
