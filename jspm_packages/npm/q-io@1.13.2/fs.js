...
(function(process) {
  ...
  exports.open = dampen(function(path, flags, charset, options) {
    ...
    var nodeOptions = {"flags": nodeFlags};
  ...
  exports.rename = function(source, target) {
    ...
    return Q.ninvoke(FS, "rename", source, target).fail(function(error) {
      if (error.code === "EXDEV") {
        error.message = "source and target are on different devices: " + error.message;
        error.crossDevice = true;
      }
      error.message = ("Can't move " + JSON.stringify(source) + " to " + JSON.stringify(target) + " because " + error.message);
      throw error;
    });
  };
