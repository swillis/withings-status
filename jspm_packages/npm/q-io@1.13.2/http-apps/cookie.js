...
exports.CookieJar = function(app) {
  ...
  return function(request) {
    ...
    var requestCookies = concat(Object.keys(hostCookies).map(function(host) {
      if (!hostContains(host, request.headers.host)) {
        return [];
      }
      var pathCookies = hostCookies[host];
      return concat(Object.keys(pathCookies).map(function(path) {
        ...
        return (Object.keys(cookies).map(function(name) {
          ...
          return cookie.secure ? request.ssl : true;
        }));
      }));
    }));
    if (requestCookies.length) {
      request.headers["cookie"] = (requestCookies.map(function(cookie) {
        return Cookie.stringify(cookie.key, cookie.value);
      }).join("; "));
    }
    return Q.when(app.apply(this, arguments), function(response) {
      ...
      if (response.headers["set-cookie"]) {
        ...
        response.headers["set-cookie"].forEach(function(cookie) {
          var date = response.headers["date"] ? new Date(response.headers["date"]) : new Date();
...
function hostContains(containerHost, contentHost) {
  ...
  if (ipRe.test(containerHostname) || ipRe.test(contentHostname)) {
    ...
    return (contentHostname.lastIndexOf(containerHostname) === contentHostname.length - containerHostname.length) || (containerHostname.slice(1) === contentHostname);
  } else {
    return containerHostname === contentHostname;
  }
}
...
function pathContains(container, content) {
  if (/^\/$/.test(container)) {
    ...
    return (content === container || content.indexOf(container + "/") === 0);
  }
}
