...
exports.FileTree = function(root, options) {
  ...
  return function(request, response) {
    ...
    request.fs = fs;
    var redirect = options.redirect || (request.permanent || options.permanent ? RedirectApps.permanentRedirect : RedirectApps.temporaryRedirect);
...
exports.file = function(request, path, contentType, fs) {
  ...
  return Q.when(fs.stat(path), function(stat) {
    ...
    if ("range" in request.headers) {
      if ("if-range" in request.headers && etag != request.headers["if-range"]) {} else {
        ...
        if (range) {
          ...
          if (range.end <= range.begin) {
            return StatusApps.responseForStatus(request, 416);
          }
          status = 206;
          headers["content-range"] = ("bytes " + range.begin + "-" + (range.end - 1) + "/" + stat.size);
...
exports.etag = function(stat) {
  return [stat.node.ino, stat.size, stat.lastModified().getTime()].join("-");
};
...
exports.listDirectoryHtmlFragment = function(request, response) {
  return exports.listDirectoryData(request, response).then(function(data) {
    return {
      status: 200,
...
exports.listDirectoryData = function(request, response) {
  ...
  var fs = request.fs;
  return Q.invoke(fs, "list", response.directory).then(function(list) {
    list.sort();
    return list.map(function(name) {
      return Q.invoke(fs, "stat", fs.join(response.directory, name)).then(function(stat) {
        if (stat.isDirectory()) {
          return {
            name: name,
            stat: {type: "directory"}
      ...
      return Q.fcall(app, request).then(function(response) {
        if (response.directory !== void 0) {
          if (request.location.file) {
            ...
            return Q.invoke(request.fs, "isFile", index).then(function(isFile) {
              if (isFile) {
